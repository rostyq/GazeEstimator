...
from model import estimate_gaze
...
def get_face_pose(rects, gray, frame, predictor, model):
	...
	for person_face in image_points:
		...
		(end_points2D, jacobian) = cv2.projectPoints(four_points_plane, rotation_vector, translation_vector, camera_matrix, dist_coeffs)
...
#                  (int(end_points2D[0][0][0]), int(end_points2D[0][0][1])), (255,0,0), 2) 
		...
		#gaze estimation
		gaze = estimate_gaze(np.array([left_eye_frame/255, np.flip(right_eye_frame/255, axis = 1)]), np.array([rotation_vector, rotation_vector * np.array([[-1], [1], [1]])]))
		print(right_eye_frame.shape)
		cv2.putText(frame, str(gaze), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

		left_eye_center = np.array([150.0, 170.0, -135.0])
		left_eye_gaze = left_eye_center + 1000 * gaze[0]
		end_points, _ = cv2.projectPoints(np.array([left_eye_center, left_eye_gaze]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
		cv2.line(frame, (int(end_points[0][0][0]), int(end_points2D[0][0][1])),
					  (int(end_points[1][0][0]), int(end_points2D[1][0][1])), (255,0,0), 2)
					  
		right_eye_center = np.array([-150.0, 170.0, -135.0])
		right_eye_gaze = right_eye_center + 1000 * gaze[1]
		end_points, _ = cv2.projectPoints(np.array([right_eye_center, right_eye_gaze]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
		cv2.line(frame, (int(end_points[0][0][0]), int(end_points2D[0][0][1])),
						(int(end_points[1][0][0]), int(end_points2D[1][0][1])), (255,0,0), 2)
